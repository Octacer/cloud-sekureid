...
Two ways in, same engine:
  • POST /extract-text        — JSON { url } : the server downloads a publicly reachable file.
  • POST /extract-text-file   — multipart file upload : the bytes are sent directly (no public URL
                                needed). Preferred when the caller already holds the file (e.g. a
                                screenshot) — it avoids a storage round-trip and viewer-page pitfalls.
"""
...
def _process_saved_file(temp_raw_file, request_id, background_tasks) -> TextExtractionResponse:
    """
    Detect the type of an already-saved raw file and extract its text. Shared by both the URL
    endpoint (which downloads first) and the file-upload endpoint (which saves the bytes first),
    so the two entry points always behave identically. Schedules cleanup of the temp file.
    """
    ...
    temp_file = os.path.join(EXTRACT_TEMP_DIR, f"{request_id}.{file_extension}")
...
async def extract_text(
    ...
    """
    Extract text from an image, PDF, spreadsheet, or Word document reachable at a public URL.
    ...
    try:
        print(f"\n{'='*80}")
        print(f"[{request_id}] Processing text extraction request (URL)")
        print(f"{'='*80}")
        ...
        return _process_saved_file(temp_raw_file, request_id, background_tasks)
...
@router.post("/extract-text-file", response_model=TextExtractionResponse)
async def extract_text_file(
    background_tasks: BackgroundTasks,
    file: UploadFile = File(...)
):
    """
    Extract text from an uploaded file (image, PDF, spreadsheet, or Word document) — the bytes are
    sent directly as multipart/form-data, so no public URL is required.

    Parameters:
    - file: the document to extract (multipart form field 'file')
    ...
    try:
        print(f"\n{'='*80}")
        print(f"[{request_id}] Processing text extraction request (FILE)")
        print(f"{'='*80}")
        print(f"[{request_id}] Filename: {file.filename}  content-type: {file.content_type}")

        # Save the uploaded bytes to disk (streamed, so large files don't blow up memory).
        print(f"[{request_id}] Step 1: Saving uploaded file...")
        temp_raw_file = os.path.join(temp_extract_dir, f"{request_id}_raw")
        with open(temp_raw_file, "wb") as out:
            shutil.copyfileobj(file.file, out)
        file_size = os.path.getsize(temp_raw_file)
        print(f"[{request_id}] File saved: {file_size} bytes on disk")
